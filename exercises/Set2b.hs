...
binomial n 0 = 1
binomial n k = if n == 0 && k > 0 then 0
...
oddFactorial 1 = 1
oddFactorial n = if even n then oddFactorial (n - 1)
                 else n * oddFactorial(n - 2)
...
leftpad str x = if length str >= x then str
                else leftpad (" " ++ str) x
...
smallestDivisor x = if (x `mod` 2) == 0 then 2
                    else smallestDivisorHelper x 3
smallestDivisorHelper x n = if (x `mod` n) == 0 then n
                            else smallestDivisorHelper x (n+1)
...
biggestPrimeAtMost x = if (isPrime x) then x
